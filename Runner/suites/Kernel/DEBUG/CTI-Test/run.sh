...
cleanup() {
    if [ -f "$lpm_sleep" ] && [ -n "$sleep_value" ]; then
        log_info "Restoring LPM Sleep value: $sleep_value"
        echo "$sleep_value" > "$lpm_sleep" 2>/dev/null
    fi
}
trap cleanup EXIT
...
setup_sleep() {
    if [ -f "$lpm_sleep" ]; then
        sleep_value=$(cat "$lpm_sleep" 2>/dev/null)
        if [ "$sleep_value" != "Y" ] && [ "$sleep_value" != "1" ]; then
